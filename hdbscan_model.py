...
HDBSCAN baseline with the same data handling & metrics as GOTHIC/KMeans/DBSCAN,
so we can compare them fairly.

Pipeline:
  1) Load dataset via loader.load_dataset.
  2) Train/test split (random or density-balanced).
  3) Fit HDBSCAN on the train set.
  4) Predict clusters for train & test:
       - train: HDBSCAN labels (with -1 as noise)
       - test : nearest-neighbor projection from train labels
  5) Compute metrics:
       - NMI, AMI, ARI, FMI, ACC
...
# Local imports (same as in kmeans_model/dbscan_model/gothic_model)
from helper import (
    set_seed,
...
DEFAULT_MIN_SAMPLES = 5  # if <= 0, we let HDBSCAN decide (None)


# =========================== ARG PARSER ============================

def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="HDBSCAN baseline for clustering.")
    ...
    parser.add_argument(
        ...
        help="min_samples parameter for HDBSCAN; if <= 0, let HDBSCAN choose.",
    )
    parser.add_argument(
        "--cluster_selection_epsilon",
        type=float,
        default=0.0,
        help="cluster_selection_epsilon for HDBSCAN (0.0 means disabled).",
    )
    parser.add_argument(
        ...
        default="eom",
        choices=["eom", "leaf"],
        help="cluster_selection_method for HDBSCAN.",
    )
...
def train_test_split_with_strategy(X, y, train_frac=0.8, strategy="balanced", seed=42):
    """
    Same behavior as in gothic_model/kmeans_model/dbscan_model:
...
def safe_cluster_metric(fn, X_data, labels_pred, name: str) -> float:
    ...
    """
    try:
        # Need at least 2 distinct labels for these metrics to make sense.
...
def hdbscan_main(args=None) -> Dict[str, Any]:
    if args is None:
        args = parse_args()
    ...
    # HDBSCAN parameters
    min_samples = None if args.min_samples is not None and args.min_samples <= 0 else args.min_samples
    epsilon = args.cluster_selection_epsilon if args.cluster_selection_epsilon > 0.0 else 0.0

    print(
        "[INFO] Fitting HDBSCAN on train set "
    ...
    y_pred_train = hdb.labels_  # includes -1 for noise

    # Assign test labels by nearest train point's cluster label
    if len(X_train) > 0:
    ...
    # Accuracy with permutation (noise treated as another cluster)
    ...
    print(f"[RESULT] Dataset = {dataset} (HDBSCAN)")
    ...
    # (b) Train HDBSCAN clusters (title: NMI & ACC only)
    ax = axes[1]
    ax.scatter(
        train_x,
        train_y,
    ...
    ax.set_title(f"Train HDBSCAN (NMI={nmi_train:.3f}, ACC={acc_train:.3f})")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ...
    ax.set_title(f"Test HDBSCAN (NMI={nmi_test:.3f}, ACC={acc_test:.3f})")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.legend(loc="best", fontsize=8)

    fig.tight_layout()
    train_test_path = figures_dir / f"{dataset}_hdbscan_train_test.png"
    fig.savefig(train_test_path, dpi=150, transparent=True)
    print(f"[INFO] Saved HDBSCAN train/test figure to: {train_test_path}")
    if args.show_plots:
        fig.show()
    else:
        plt.close(fig)
    ...
    try:
        ...
        full_gt_path = figures_dir / f"{dataset}_hdbscan_full_ground_truth.png"
        fig_full.savefig(full_gt_path, dpi=150, transparent=True)
        print(f"[INFO] Saved full dataset GT figure (HDBSCAN run) to: {full_gt_path}")
        if args.show_plots:
            fig_full.show()
        else:
            plt.close(fig_full)
    ...
    try:
        ...
        for name, (v_train, v_test) in metric_values.items():
            ...
            ax_m.set_title(f"{name} (HDBSCAN, train vs test)")
            ax_m.set_ylabel(name)
            fig_m.tight_layout()
            metric_path = figures_dir / f"{dataset}_hdbscan_metric_{name.lower()}.png"
            fig_m.savefig(metric_path, dpi=150, transparent=True)
            print(f"[INFO] Saved HDBSCAN metric plot '{name}' to: {metric_path}")
            if args.show_plots:
                fig_m.show()
            else:
                plt.close(fig_m)
    except Exception as e:
        print(f"[WARN] Failed to save HDBSCAN metric bar plots: {e}")

    # ================ SAVE TEXT RESULTS / METRICS ==================

    results_txt_path = out_dir / f"{dataset}_hdbscan_results.txt"
    with open(results_txt_path, "w") as f:
        ...
        f.write(f"cluster_selection_eps  = {epsilon}\n")
        f.write(f"cluster_selection_meth = {args.cluster_selection_method}\n")
        f.write("\n")
    ...
    print(f"[INFO] Saved HDBSCAN scalar results to: {results_txt_path}")
    ...
    row = {
        ...
        "min_cluster_size": args.min_cluster_size,
        "min_samples": min_samples,  # can be None
