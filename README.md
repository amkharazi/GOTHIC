...
- A **graph** over micro‑clusters (edges based on centroid and boundary distances),
...
It’s designed to explore graph + neural approaches to clustering on both synthetic and real datasets.
...
3. **Node features**
   - For each micro‑cluster, we build a feature vector:
     ...
     - `radius_mean`
     - `radius_std`
     - `log_density` (size divided by a volume proxy, then log).
...
5. **Hierarchical merging**
   ...
   - Sort edges by **decreasing** predicted probability.
...
6. **Assigning labels to points**
   - For train points: each point belongs to a micro‑cluster → map it to its macro‑cluster.
   - For test points: assign each to the nearest **effective** micro‑cluster (the ones that actually appear after removing empty ones), then map that micro‑cluster to a macro‑cluster.

7. **Evaluation & visualization**
   - Compute **NMI** on train/test.
   - Compute **clustering accuracy** with optimal label permutation (Hungarian).
   - Plot:
     - Train ground‑truth clusters,
     - Train predicted clusters,
     - Test points (correct vs mis‑clustered),
     - Micro‑cluster graph.
...
This is the main script implementing the GOTHIC pipeline.
...
outputs/
  ID001/
    ...
    compound_micro_graph.png                     # micro-cluster graph
    compound_train_test.png                      # train/test scatter plots
    compound_results.txt                         # metrics
```

`compound_results.txt` contains:

- dataset, run_id
- seed, split_strategy, train_fraction
- n_micro, k_target, dist_quantile
- Train and test NMI & ACC
