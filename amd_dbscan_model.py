...
Idea:
  ...
  - Define a scaled distance matrix:
        D'_{ij} = D_{ij} / max(d_k(i), d_k(j))
    where d_k(i) is the distance from i to its k-th nearest neighbor.
  - Run DBSCAN on D' (metric='precomputed') so a single eps can handle
    multi-density structure better.
...
Pipeline:
  ...
  3) Compute D_train, local scales, D'_train.
...
DEFAULT_K_SCALE = 15  # k for local scaling (k-th neighbor distance)
...
def amd_dbscan_main(args=None) -> Dict[str, Any]:
    if args is None:
        args = parse_args()
    ...
    if n_train == 0:
        print("[WARN] Empty train set; skipping clustering.")
        y_pred_train = np.array([], dtype=int)
        y_pred_test = np.array([], dtype=int)
    else:
        print(f"[INFO] Computing distance matrix for train (n={n_train}) ...")
        D_train = compute_distance_matrix(X_train)

        print(f"[INFO] Computing local scales with k_scale={args.k_scale} ...")
        scales = compute_local_scales(D_train, args.k_scale)

        print("[INFO] Building scaled distance matrix D' ...")
    ...
    print(f"[RESULT] Dataset = {dataset} (AMD-DBSCAN)")
    ...
    try:
        ...
        for name, (v_train, v_test) in metric_values.items():
            ...
            ax_m.set_title(f"{name} (AMD-DBSCAN, train vs test)")
    ...
    with open(results_txt_path, "w") as f:
        ...
        f.write(f"k_scale          = {args.k_scale}\n")
        f.write("\n")
