...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="DBSCAN baseline for clustering.")
...
def train_test_split_with_strategy(X, y, train_frac=0.8, strategy="balanced", seed=42):
    """
    Same behavior as in gothic_model/kmeans_model:
...
def dbscan_main(args=None) -> Dict[str, Any]:
    if args is None:
        args = parse_args()
    ...
    # Fit DBSCAN on train
    print(f"[INFO] Fitting DBSCAN on train set (eps={args.eps}, min_samples={args.min_samples}) ...")
    db = DBSCAN(
        eps=args.eps,
    ...
    y_pred_train = db.labels_  # can include -1 for noise
    ...
    print(f"[RESULT] Dataset = {dataset} (DBSCAN)")
    ...
    # (b) Train DBSCAN clusters (title: NMI & ACC only)
    ax = axes[1]
    ax.scatter(
        train_x,
        train_y,
    ...
    ax.set_title(f"Train DBSCAN (NMI={nmi_train:.3f}, ACC={acc_train:.3f})")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ...
    ax.set_title(f"Test DBSCAN (NMI={nmi_test:.3f}, ACC={acc_test:.3f})")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.legend(loc="best", fontsize=8)

    fig.tight_layout()
    train_test_path = figures_dir / f"{dataset}_dbscan_train_test.png"
    fig.savefig(train_test_path, dpi=150, transparent=True)
    print(f"[INFO] Saved DBSCAN train/test figure to: {train_test_path}")
    if args.show_plots:
        fig.show()
    else:
        plt.close(fig)
    ...
    try:
        ...
        full_gt_path = figures_dir / f"{dataset}_dbscan_full_ground_truth.png"
        fig_full.savefig(full_gt_path, dpi=150, transparent=True)
        print(f"[INFO] Saved full dataset GT figure (DBSCAN run) to: {full_gt_path}")
        if args.show_plots:
            fig_full.show()
        else:
            plt.close(fig_full)
    ...
    try:
        ...
        for name, (v_train, v_test) in metric_values.items():
            ...
            ax_m.set_title(f"{name} (DBSCAN, train vs test)")
            ax_m.set_ylabel(name)
            fig_m.tight_layout()
            metric_path = figures_dir / f"{dataset}_dbscan_metric_{name.lower()}.png"
            fig_m.savefig(metric_path, dpi=150, transparent=True)
            print(f"[INFO] Saved DBSCAN metric plot '{name}' to: {metric_path}")
            if args.show_plots:
                fig_m.show()
            else:
                plt.close(fig_m)
    except Exception as e:
        print(f"[WARN] Failed to save DBSCAN metric bar plots: {e}")

    # ================ SAVE TEXT RESULTS / METRICS ==================

    results_txt_path = out_dir / f"{dataset}_dbscan_results.txt"
